...
{
    ARLOGi("Opening ARToolKit video using configuration '%s'.\n", videoConfiguration);
...
bool ARVideoSource::captureFrame()
{
    ...
    if (deviceState == DEVICE_RUNNING) {
        if (0 != count) {
            ARLOGi("ARVideoSource::captureFrame() counted %d calls before DEVICE_RUNNING state\n", count);
            count = 0;
        }
        ...
        if (count == 0) {
            ARLOGw("Warning: unable to capture frame, waiting for video source open step 3 to complete (before going to run state)\n");
        }
        count++;
    }
    return false;
...
bool ARVideoSource::close()
{
    ...
    if (deviceState == DEVICE_RUNNING) {
        ARLOGd("ARVideoSource::close(): stopping video, calling ar2VideoCapStop(m_vid).\n");
    ...
    ARLOGi("Closing ARToolKit video.\n");
